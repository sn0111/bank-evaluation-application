...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(BankEvaluationException.class)
    public ResponseEntity<ResponseDTO> handleEmrServiceException(BankEvaluationException ex) {
        log.error("BankEvaluationException error message: {}", ex.getMessage());
        ResponseDTO responseDto = ResponseBuilder.buildErrorResponse(String.valueOf(HttpStatus.BAD_REQUEST.value()), ex.getMessage());
        return new ResponseEntity<>(responseDto, HttpStatus.BAD_REQUEST);
    }
